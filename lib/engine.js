...
function buildOutputValues(config, meterValue, dayStartValue, ledger, monthStartValue) {
    ...
    const monat = calcPeriodSummaryByMeterBounds(
        config,
        startOfMonth(now),
        monthStart,
        now,
    ...
    const letzterMonat = sumLedgerBetween(
        config,
        ledger,
        startOfMonth(prevMonthRef),
        endOfMonth(prevMonthRef)
    );

    // Aktuelles Abrechnungsjahr = aktueller Tarifblock
    ...
    // Letztes Abrechnungsjahr = vorheriger Tarifblock
