...
class EnergyGas extends utils.Adapter {
    constructor(options = {}) {
        ...
        this.ownStateIds = new Set(['consumption.meter_reading']);
        this.lastPulseTs = 0;
    ...
    async onReady() {
        ...
        if (!tariffs.length) {
            this.log.warn('Adapter not active yet: No active tariff available');
            return;
        }
    ...
    async onStateChange(id, state) {
        ...
        if (id === `${this.namespace}.consumption.meter_reading` && state.ack !== true) {
            try {
    ...
    getMissingInputMessage() {
        return this.getSourceType() === 'counter'
            ? 'No counter data point configured'
            : 'No gas meter data point configured';
    }

    getInvalidInputMessage() {
        const inputStateId = this.getConfiguredInputStateId();
        return this.getSourceType() === 'counter'
            ? `Counter data point does not provide a valid value (${inputStateId})`
            : `Gas meter data point does not provide a valid value (${inputStateId})`;
    }
    ...
    async getDetectedCounterType(rawValue = undefined) {
        ...
        const existing = await this.getStateAsync('_internal.counter_detected_type');
        ...
        if (detected) {
            await this.setStateAsync('_internal.counter_detected_type', { val: detected, ack: true });
    ...
    async getCounterOffset() {
        const state = await this.getStateAsync('_internal.counter_offset_m3');
        return state ? parseNum(state.val, 0) : 0;
    }

    async setCounterOffset(value) {
        await this.setStateAsync('_internal.counter_offset_m3', { val: value, ack: true });
    }

    async getPulseTotal() {
        const state = await this.getStateAsync('_internal.counter_pulse_total');
        return state ? parseNum(state.val, 0) : 0;
    }

    async setPulseTotal(value) {
        await this.setStateAsync('_internal.counter_pulse_total', { val: value, ack: true });
    }
    ...
    async initCounterBaselineIfNeeded() {
        ...
        if (detectedType === 'numeric') {
            const lastRawState = await this.getStateAsync('_internal.counter_last_raw');
            const hasLastRaw =
                lastRawState &&
            ...
            if (!hasLastRaw) {
                await this.setStateAsync('_internal.counter_last_raw', {
                    val: String(parseNum(rawValue, 0)),
                    ack: true,
                });
            }
            return;
        ...
        const hasLastRaw =
            ...
            String(lastRawState.val) !== '';
        if (!hasLastRaw) {
            await this.setStateAsync('_internal.counter_last_raw', { val: String(rawValue === true), ack: true });
        }
    }
    ...
    async processCounterInputChange(rawValue) {
        ...
        if (detectedType === 'numeric') {
            ...
            if (!Number.isFinite(currentRaw)) {
                throw new Error('Counter data point does not provide a numeric value');
            }
            ...
            if (Number.isFinite(lastRaw) && currentRaw < lastRaw) {
                ...
                this.log.warn(
                    `Numeric counter decreased (${lastRaw} -> ${currentRaw}). Offset was adjusted automatically to ${newOffset} m³ so the meter reading does not jump backwards.`,
                );
            }

            await this.setStateAsync('_internal.counter_last_raw', { val: String(currentRaw), ack: true });
            await this.setStateAsync('consumption.counter', { val: currentRaw, ack: true });
            return;
        }

        if (typeof normalized !== 'boolean') {
            throw new Error('Counter data point does not provide a boolean value');
        }
        ...
        await this.setStateAsync('_internal.counter_last_raw', { val: String(normalized), ack: true });
    }
    ...
    async handleManualMeterCorrection(targetValue) {
        ...
        const detectedType = await this.getDetectedCounterType(rawValue);
        if (!detectedType) {
            throw new Error('Counter type could not be detected');
        }
        ...
        await this.setCounterOffset(newOffset);
        this.log.info(`Counter offset set to ${newOffset} m³. New meter reading: ${desiredMeter} m³`);
    ...
    async initDayStartIfNeeded(currentMeter) {
        ...
        const dayStartDateState = await this.getStateAsync('_internal.day_start_date');
        const savedDate = dayStartDateState && dayStartDateState.val ? String(dayStartDateState.val) : '';

        if (!savedDate) {
            await this.setStateAsync('_internal.day_start', { val: currentMeter, ack: true });
            await this.setStateAsync('_internal.day_start_date', { val: todayKey, ack: true });
            return;
        }
    ...
    async initMonthStartIfNeeded(currentMeter) {
        ...
        const monthMarkerState = await this.getStateAsync('_internal.month_marker');
        const monthStartState = await this.getStateAsync('_internal.month_start');

        const marker = monthMarkerState?.val ? String(monthMarkerState.val) : '';
        const existingMonthStart = monthStartState?.val;

        if (!marker) {
            await this.setStateAsync('_internal.month_start', {
                val:
            ...
            await this.setStateAsync('_internal.month_marker', {
    ...
    async handleMonthRollover(currentMeter) {
        ...
        if (now.getDate() === 1) {
            const dayStartState = await this.getStateAsync('_internal.day_start');
            if (dayStartState && dayStartState.val !== null && dayStartState.val !== undefined) {
                newMonthStart = parseNum(dayStartState.val, currentMeter);
            }
        }

        await this.setStateAsync('_internal.month_start', { val: newMonthStart, ack: true });
        await this.setStateAsync('_internal.month_marker', { val: currentMonthMarker, ack: true });

        this.log.debug(`Month rollover detected, new month start: ${newMonthStart} m³`);
    }
    ...
    async handleAutoCreateNextTariff(currentMeter) {
        ...
        if (currentIndex >= 0) {
            ...
            if (currentStartMeter <= 0 && !shouldSkipAutoSetForCounter) {
                ...
                this.log.info(
                    `Start meter reading for active tariff ${rawCurrentTariff.name || fmtDate(currentTariff.start)} ` +
                        `(${fmtDate(currentTariff.start)}) set automatically to ${currentMeter}`,
                );
            } else if (currentStartMeter <= 0 && shouldSkipAutoSetForCounter) {
                this.log.info(
                    'Start meter reading of the active tariff was not set automatically in counter mode ' +
                        'because the calculated meter reading is currently 0.',
                );
            }
        }
        ...
        tariffRows.push({
            aktiv: true,
            name: currentTariff.name || 'Tariff',
        ...
        this.log.info(
            `Follow-up tariff created automatically: ${fmtDate(nextStart)} ` +
                `(start meter reading remains empty until this tariff becomes active)`,
        );
    }
    ...
    async handleValueUpdate(meterValue) {
        ...
        const dayStartState = await this.getStateAsync('_internal.day_start');
        const dayStartDateState = await this.getStateAsync('_internal.day_start_date');
        const ledgerState = await this.getStateAsync('_internal.ledger_json');
        ...
        if (dayStartDate !== todayKey) {
            ...
            await this.setStateAsync('_internal.ledger_json', { val: JSON.stringify(ledger), ack: true });
            await this.setStateAsync('_internal.day_start', { val: meterValue, ack: true });
            await this.setStateAsync('_internal.day_start_date', { val: todayKey, ack: true });
        }
        ...
        const currentDayStartState = await this.getStateAsync('_internal.day_start');
        const currentMonthStartState = await this.getStateAsync('_internal.month_start');
